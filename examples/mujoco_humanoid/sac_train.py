...
if __name__ == '__main__':
    ...
    total_timesteps = 24000000  # 总训练步数
    eval_step = 80000  # 每多少步评估一次
    n_eval_episodes = 10  # 每次评估多少个回合


    ## 创建并行环境
    env = SubprocVecEnv([make_env(ENV_ID, i, SEED) for i in range(NUM_ENVS)])
    env = VecMonitor(env)  # 配合VecMonitor使用
    env = VecNormalize(env, norm_obs=True, norm_reward=True)  # 训练环境归一化
    ...
    eval_env = VecNormalize(eval_env, norm_obs=True, norm_reward=True, training=False)  # 不更新统计量
    ...
    model = SAC(
        ...
        buffer_size=800_000,      # 经验回放缓冲区大小. 这个参数PPO没有
